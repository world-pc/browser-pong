# nodejs-pong
a multiplayer browser pong game written in nodejs.

## to-do for v0.5

* ~~feature: implement pong bot~~ 
* ~~feature: username highlighting (@ too) in chat.~~
* ~~feature: prevent challenges to and from players in matches~~
* ~~feature: indicate who is and isn't in a match.
* feature: put lobby user list in a scrollable container.
* feature: indicate whom a player is in a match with.
* feature: forfeit button.
* feature: spectate mode.
* ~~feature: randomize which side the ball is served to.~~
* ~~bug: sometimes the submenu on another user doesn't appear.~~
* ~~bug: THREE isn't loading in chromium incognito windows..~~ 
* bug: I think the wrong player's getting the win score update.
